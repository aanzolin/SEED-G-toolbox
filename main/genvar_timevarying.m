%% Function genvar_timevarying
%       Modified version of the function genvar from MVGC toolbox [1] 
%       Modified on  May 31 2019
%% @modified from alessandra anzolin (aanzolin@mgh.harvard.edu)
% 
...
%
%%% A = ModelDel (ch x ch x popt x samp)

function [X,E] = genvar_timevarying(A,E,trunc)

global have_genvar_mex;

assert(isreal(A) && (ndims(A) == 3 || ndims(A) == 4), 'VAR coefficients must be a 3D or 4D matrix')
