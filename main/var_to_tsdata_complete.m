%% var_to_tsdata_complete
% Alessandra Anzolin 31/05/2016
% If the input "A" in 4-D automatically the function "genvar_timeVarying"
% will be used
% 
...
function [X,E,mtrunc] = var_to_tsdata_complete(A,SIG,m,N,mtrunc,decayfac)
...
if N > 1 % multi-trial
    ...
    for r = 1:N
        if length(size(A))==3
            ...
            [X(:,:,r),E(:,:,r)] = genvar_timevarying(A,C*randn(n,m+mtrunc),mtrunc);
        end
    end
    ...
    if length(size(A))==3
        ...
        [X,E] = genvar_timevarying(A,C*randn(n,m+mtrunc),mtrunc);
    end
